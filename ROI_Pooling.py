...
def ROI_Pooling(feature_map, output_shape):   # Feature map: C x h x w, output_shape: h x w
    ...
    if pad_h > 0:
        # Padding to the top
        feature_map = torch.cat((torch.zeros(channels, pad_h, width), feature_map), dim=1)
        # Padding to the bottom
        feature_map = torch.cat((feature_map, torch.zeros(channels, pad_h, width)), dim=1)
    if pad_w > 0:
        # Padding to the left
        feature_map = torch.cat((torch.zeros(channels, height, pad_w), feature_map), dim=2)
        # Padding to the right
        feature_map = torch.cat((feature_map, torch.zeros(channels, height, pad_w)), dim=2)
